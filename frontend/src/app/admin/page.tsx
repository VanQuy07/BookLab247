...
const MOCK_BOOKINGS = [
  { id: "b1", roomId: "r1", customerName: "Nguyễn Văn A", phone: "0901234567", status: "checked-in", startTime: "07:30", durationMins: 120, bufferMins: 15, note: "Mượn thêm 1 máy chiếu" },
  { id: "b2", roomId: "r1", customerName: "Lê Thị B", phone: "0987654321", status: "confirmed", startTime: "10:00", durationMins: 90, bufferMins: 15, note: "Học nhóm môn React" },
  { id: "b3", roomId: "r2", customerName: "Trần C", phone: "0911222333", status: "pending", startTime: "13:30", durationMins: 180, bufferMins: 30, note: "Thực hành Đồ án" },
];
...
export default function AdvancedAdminDashboard() {
  ...
  // ================= TẤT CẢ STATES GỘP CHUNG =================
  const [rooms, setRooms] = useState<any[]>([]);
  const [equipments, setEquipments] = useState<any[]>([]);
  const [usersList, setUsersList] = useState<any[]>([]);

  // States của nhánh Hang (Dành cho Sửa & Modal)
  ...
  // States của nhánh master (Dành cho Bộ lọc Người dùng)
  ...
  // ================= XỬ LÝ ẢNH & DATABASE (Từ nhánh Hang) =================
  ...
  const handleEditRoom = (room: any) => {
    ...
    setNewRoom({
      ...
      capacity: room.capacity || 0, pricePerHour: room.pricePerHour || 0, bufferTimeMinutes: room.bufferTimeMinutes || 15,
  ...
  const handleEditEquipment = (eq: any) => {
    ...
    setNewEq({
      ...
      roomId: eq.roomId || "", pricePerHour: eq.pricePerHour || 0
    });
    setShowEqForm(true);
  };
  ...
  const handleToggleLockUser = async (userId: string, currentStatus: boolean) => {
    ...
    setUsersList((prev) => prev.map((u) => (u.id || u._id) === userId ? { ...u, is_active: !currentStatus } : u));
    try {
      const response = await fetch(`${API_URL}/auth/${userId}/status`, {
        method: "PATCH", headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ is_active: !currentStatus }),
      });
      if (!response.ok) alert("Lỗi từ server: Không thể cập nhật trạng thái!");
    } catch (error) { console.error("Lỗi cập nhật trạng thái:", error); }  
  };

  const handleDeleteUser = async (userId: string) => {
    if (!window.confirm("Hành động này sẽ XÓA VĨNH VIỄN tài khoản. Bạn có chắc chắn?")) return;
    setUsersList((prev) => prev.filter((u) => (u.id || u._id) !== userId));
    try {
      const response = await fetch(`${API_URL}/auth/${userId}`, { method: "DELETE" });
      if (!response.ok) alert("Lỗi từ server: Không thể xóa tài khoản!");
  ...
  // ================= 1. GIAO DIỆN LỊCH ĐẶT PHÒNG =================
  const renderBookings = () => {
    const startHour = 7; const endHour = 21; const totalMins = (endHour - startHour) * 60;
    const timeHeaders = [];
    ...
    const roomsWithBookings = rooms.filter((room) => MOCK_BOOKINGS.some((booking) => booking.roomId === room.id));
        ...
        <div className="flex-1 bg-white border border-gray-200 rounded-2xl overflow-auto relative shadow-sm">
          <div className="min-w-[1200px] h-full flex flex-col">
            <div className="flex sticky top-0 z-20 bg-slate-50 border-b border-gray-200 shadow-sm">
              <div className="w-64 shrink-0 sticky left-0 z-30 bg-slate-50 border-r border-gray-200 p-4 font-black text-gray-700 flex items-center justify-between">Danh sách Phòng</div>
                ...
                return (
                  <div key={room.id} className="flex border-b border-gray-100 group hover:bg-slate-50/50 transition-colors h-24">
                    <div className="w-64 shrink-0 sticky left-0 z-10 bg-white group-hover:bg-slate-50 border-r border-gray-200 p-4 flex flex-col justify-center">
                      ...
                      <p className="text-xs text-gray-500 font-medium flex items-center gap-1 mt-1"><MapPin className="w-3 h-3" /> {room.building} - {room.floor}</p>
                      <p className="text-xs text-gray-400 mt-0.5"><Users className="w-3 h-3 inline" /> {room.capacity} người</p>
                    </div>
                    <div className="flex-1 relative bg-[url('data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAAIAAAACCAYAAABytg0kAAAAGXRFWHRTb2Z0d2FyZQBBZG9iZSBJbWFnZVJlYWR5ccllPAAAABZJREFUeNpi2rVq1X8GBgYQwgcsAAgwAA9GA9/5o7wLAAAAAElFTkSuQmCC')] cursor-pointer" onClick={() => alert(`Bật Form Đặt lịch nhanh cho phòng: ${room.name}`)}>
                      {roomBookings.map((booking) => {
                        ...
                        return (
                          <React.Fragment key={booking.id}>
                            <div className={`absolute top-2 bottom-2 ${getStatusColor(booking.status)} border rounded-l-lg rounded-r-sm p-2 shadow-sm cursor-move overflow-hidden z-10 flex flex-col justify-center group/block`} style={{ left: `${leftPct}%`, width: `${widthPct}%` }} onClick={(e) => { e.stopPropagation(); alert(`Mở popup chỉnh sửa ca của ${booking.customerName}`); }}>
                            ...
                            {booking.bufferMins > 0 && (
                              <div className="absolute top-2 bottom-2 z-0 bg-gray-200 border-y border-r border-gray-300 rounded-r-lg opacity-70 flex items-center justify-center overflow-hidden cursor-not-allowed" style={{ left: `${leftPct + widthPct}%`, width: `${bufferWidthPct}%`, backgroundImage: "repeating-linear-gradient(45deg, transparent, transparent 5px, rgba(0,0,0,0.05) 5px, rgba(0,0,0,0.05) 10px)" }} title={`Thời gian dọn dẹp ${booking.bufferMins} phút`}>
  ...
  const renderDashboard = () => {
    const dashboardData = {
      yesterday: { metrics: [{ title: "Trạng thái phòng", value: "60%", subValue: "12/20 phòng sử dụng", icon: Activity, trend: "down" as const, colorClass: "text-blue-600", bgClass: "bg-blue-50" }, { title: "Đơn đặt lịch", value: "85", subValue: "Hoàn tất 85 đơn", icon: CalendarDays, trend: "down" as const, colorClass: "text-emerald-600", bgClass: "bg-emerald-50" }, { title: "Thiết bị cho mượn", value: "24", subValue: "Đã trả 5 thiết bị", icon: MonitorPlay, trend: "down" as const, colorClass: "text-amber-600", bgClass: "bg-amber-50" }, { title: "Doanh thu (Ngày)", value: "2.1M", subValue: "-5% so với hôm kia", icon: CreditCard, trend: "down" as const, colorClass: "text-purple-600", bgClass: "bg-purple-50" }], peakHours: [{ time: "07:00", bookings: 5 }, { time: "09:00", bookings: 25 }, { time: "11:00", bookings: 20 }, { time: "13:00", bookings: 35 }, { time: "15:00", bookings: 30 }, { time: "17:00", bookings: 10 }], popularRooms: [{ name: "Lab Máy tính", value: 30 }, { name: "Lab Hóa - Sinh", value: 40 }, { name: "Phòng Hội thảo", value: 10 }, { name: "Không gian chung", value: 20 }] },
      today: { metrics: [{ title: "Trạng thái phòng", value: "75%", subValue: "15/20 phòng đang dùng", icon: Activity, trend: "up" as const, colorClass: "text-blue-600", bgClass: "bg-blue-50" }, { title: "Đơn đặt lịch", value: "124", subValue: "12 đơn chờ duyệt", icon: CalendarDays, trend: "up" as const, colorClass: "text-emerald-600", bgClass: "bg-emerald-50" }, { title: "Thiết bị cho mượn", value: "45", subValue: "Trong tổng số 320 TB", icon: MonitorPlay, trend: "neutral" as const, colorClass: "text-amber-600", bgClass: "bg-amber-50" }, { title: "Doanh thu (Ngày)", value: "3.2M", subValue: "+12.5% so với hôm qua", icon: CreditCard, trend: "up" as const, colorClass: "text-purple-600", bgClass: "bg-purple-50" }], peakHours: [{ time: "07:00", bookings: 12 }, { time: "09:00", bookings: 45 }, { time: "11:00", bookings: 30 }, { time: "13:00", bookings: 55 }, { time: "15:00", bookings: 60 }, { time: "17:00", bookings: 25 }], popularRooms: [{ name: "Lab Máy tính", value: 45 }, { name: "Lab Hóa - Sinh", value: 25 }, { name: "Phòng Hội thảo", value: 20 }, { name: "Không gian chung", value: 10 }] },
      "7days": { metrics: [{ title: "Trạng thái phòng", value: "82%", subValue: "Trung bình 7 ngày", icon: Activity, trend: "up" as const, colorClass: "text-blue-600", bgClass: "bg-blue-50" }, { title: "Đơn đặt lịch", value: "856", subValue: "Tăng mạnh đầu tuần", icon: CalendarDays, trend: "up" as const, colorClass: "text-emerald-600", bgClass: "bg-emerald-50" }, { title: "Thiết bị cho mượn", value: "120", subValue: "Chưa thu hồi 15 TB", icon: MonitorPlay, trend: "neutral" as const, colorClass: "text-amber-600", bgClass: "bg-amber-50" }, { title: "Doanh thu (Tuần)", value: "22.5M", subValue: "+8% so với tuần trước", icon: CreditCard, trend: "up" as const, colorClass: "text-purple-600", bgClass: "bg-purple-50" }], peakHours: [{ time: "T2", bookings: 120 }, { time: "T3", bookings: 150 }, { time: "T4", bookings: 140 }, { time: "T5", bookings: 180 }, { time: "T6", bookings: 160 }, { time: "T7", bookings: 80 }, { time: "CN", bookings: 26 }], popularRooms: [{ name: "Lab Máy tính", value: 300 }, { name: "Lab Hóa - Sinh", value: 150 }, { name: "Phòng Hội thảo", value: 250 }, { name: "Không gian chung", value: 156 }] },
      month: { metrics: [{ title: "Trạng thái phòng", value: "68%", subValue: "Trung bình tháng", icon: Activity, trend: "down" as const, colorClass: "text-blue-600", bgClass: "bg-blue-50" }, { title: "Đơn đặt lịch", value: "3,240", subValue: "Hoàn tất 98%", icon: CalendarDays, trend: "up" as const, colorClass: "text-emerald-600", bgClass: "bg-emerald-50" }, { title: "Thiết bị cho mượn", value: "840", subValue: "Mất/Hỏng 2 TB", icon: MonitorPlay, trend: "down" as const, colorClass: "text-amber-600", bgClass: "bg-amber-50" }, { title: "Doanh thu (Tháng)", value: "86.4M", subValue: "-2% so với tháng trước", icon: CreditCard, trend: "down" as const, colorClass: "text-purple-600", bgClass: "bg-purple-50" }], peakHours: [{ time: "Tuần 1", bookings: 800 }, { time: "Tuần 2", bookings: 950 }, { time: "Tuần 3", bookings: 820 }, { time: "Tuần 4", bookings: 670 }], popularRooms: [{ name: "Lab Máy tính", value: 1200 }, { name: "Lab Hóa - Sinh", value: 800 }, { name: "Phòng Hội thảo", value: 600 }, { name: "Không gian chung", value: 640 }] },
    };
      ...
      <div className="space-y-8 pb-10">
        <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-4">
          ...
          <div className="flex bg-gray-100/80 p-1 rounded-xl border border-gray-200">
            {[{ id: "yesterday", label: "Hôm qua" }, { id: "today", label: "Hôm nay" }, { id: "7days", label: "7 ngày qua" }, { id: "month", label: "Tháng này" }].map((filter) => (
              <button key={filter.id} onClick={() => setTimeFilter(filter.id as TimeFilter)} className={`px-4 py-2 text-sm font-bold rounded-lg transition-all ${timeFilter === filter.id ? "bg-white text-blue-600 shadow-sm" : "text-gray-500 hover:text-gray-900 hover:bg-gray-200/50"}`}>
                {filter.label}
              </button>
            ))}
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          {currentData.metrics.map((metric, idx) => (
            <motion.div key={`${timeFilter}-metric-${idx}`} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.2, delay: idx * 0.05 }} className="bg-white p-6 rounded-2xl border border-gray-100 shadow-sm flex items-center gap-5 hover:shadow-md transition-shadow">
              <div className={`w-14 h-14 rounded-full flex items-center justify-center shrink-0 ${metric.bgClass}`}><metric.icon className={`w-7 h-7 ${metric.colorClass}`} /></div>
              <div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          <motion.div key={`${timeFilter}-chart1`} initial={{ opacity: 0, scale: 0.98 }} animate={{ opacity: 1, scale: 1 }} transition={{ duration: 0.3 }} className="bg-white p-6 rounded-2xl border border-gray-100 shadow-sm lg:col-span-2">
            <div className="mb-6"><h3 className="text-lg font-bold text-gray-900">Tần suất đặt phòng</h3><p className="text-sm text-gray-500">Giúp bố trí nhân sự kỹ thuật & lao công hợp lý.</p></div>
              ...
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={currentData.peakHours} margin={{ top: 10, right: 10, left: -20, bottom: 0 }}>
                  ...
                  <Bar dataKey="bookings" name="Số ca đặt" fill="#3b82f6" radius={[6, 6, 0, 0]} barSize={40}>
                    {currentData.peakHours.map((entry, index) => <Cell key={`cell-${index}`} fill={index === currentData.peakHours.reduce((maxIdx, current, idx, arr) => current.bookings > arr[maxIdx].bookings ? idx : maxIdx, 0) ? "#2563eb" : "#93c5fd"} />)}
                  </Bar>
                </BarChart>
              </ResponsiveContainer>
            </div>
          </motion.div>
          <motion.div key={`${timeFilter}-chart2`} initial={{ opacity: 0, scale: 0.98 }} animate={{ opacity: 1, scale: 1 }} transition={{ duration: 0.3, delay: 0.1 }} className="bg-white p-6 rounded-2xl border border-gray-100 shadow-sm">
            <div className="mb-6"><h3 className="text-lg font-bold text-gray-900">Mức độ ưa chuộng</h3><p className="text-sm text-gray-500">Phân bổ tỷ lệ loại phòng được đặt.</p></div>
            <div className="h-72 w-full flex items-center justify-center">
              <ResponsiveContainer width="100%" height="100%">
                <PieChart>
                  <Pie data={currentData.popularRooms} cx="50%" cy="45%" innerRadius={60} outerRadius={90} paddingAngle={5} dataKey="value">
                    {currentData.popularRooms.map((entry, index) => <Cell key={`cell-${index}`} fill={COLORS[index % COLORS.length]} stroke="none" />)}
                  </Pie>
  ...
  // ================= 3. GIAO DIỆN PHÒNG LAB (Từ nhánh Hang) =================
  const renderRooms = () => (
  ...
  // ================= 4. GIAO DIỆN THIẾT BỊ (Từ nhánh Hang) =================
  ...
  // ================= 5. GIAO DIỆN NGƯỜI DÙNG (Từ nhánh master) =================
  const renderUsers = () => {
    const filteredUsers = usersList.filter((user) => {
      const matchNameEmail = user.full_name?.toLowerCase().includes(userSearchTerm.toLowerCase()) || user.email?.toLowerCase().includes(userSearchTerm.toLowerCase());
      const matchRole = userRoleFilter === "ALL" || user.role === userRoleFilter;
      const matchStatus = userStatusFilter === "ALL" || (userStatusFilter === "ACTIVE" && user.is_active) || (userStatusFilter === "BANNED" && !user.is_active);
      ...
      <div className="space-y-6">
        ...
        <div className="flex flex-col md:flex-row gap-4 bg-white p-4 rounded-2xl shadow-sm border border-gray-200">
          <div className="relative flex-1">
            ...
            <input type="text" placeholder="Tìm theo tên hoặc email..." value={userSearchTerm} onChange={(e) => setUserSearchTerm(e.target.value)} className="w-full pl-10 pr-4 py-2 border border-gray-200 rounded-xl focus:border-blue-500 outline-none bg-gray-50" />
          </div>
          <select value={userRoleFilter} onChange={(e) => setUserRoleFilter(e.target.value)} className="border border-gray-200 rounded-xl px-4 py-2 outline-none focus:border-blue-500 bg-gray-50 font-medium text-gray-700">
            <option value="ALL">Tất cả Quyền</option>
          </select>
          <select value={userStatusFilter} onChange={(e) => setStatusFilter(e.target.value)} className="border border-gray-200 rounded-xl px-4 py-2 outline-none focus:border-blue-500 bg-gray-50 font-medium text-gray-700">
          ...
          <table className="w-full text-left text-sm">
            <thead className="bg-gray-50 border-b border-gray-200">
              <tr>
                ...
                <th className="p-4 font-bold text-gray-700 text-right">Thao tác</th>
              </tr>
            </thead>
            <tbody>
              {filteredUsers.map((u, index) => (
                <tr key={u.id || u._id || index} className="border-b hover:bg-gray-50 transition">
                  <td className="p-4 font-bold text-gray-900">{u.full_name}</td>
                  <td className="p-4 text-gray-600">{u.email}</td>
                  <td className="p-4"><span className={`px-2.5 py-1 rounded-lg font-bold text-xs ${u.role === "ADMIN" ? "bg-purple-100 text-purple-700" : u.role === "MANAGER" ? "bg-blue-100 text-blue-700" : "bg-gray-100 text-gray-700"}`}>{u.role}</span></td>
                  <td className="p-4">{u.is_active ? <span className="text-green-600 font-semibold flex items-center gap-1.5"><ShieldCheck className="w-4 h-4" /> Hoạt động</span> : <span className="text-red-600 font-semibold flex items-center gap-1.5"><AlertTriangle className="w-4 h-4" /> Bị khóa</span>}</td>
                  <td className="p-4 text-right space-x-3">
                    <button onClick={() => handleToggleLockUser(u.id || u._id, u.is_active)} className={`${u.is_active ? "text-amber-600" : "text-green-600"} font-bold text-xs hover:underline`}>{u.is_active ? "Khóa" : "Mở khóa"}</button>
                    <button onClick={() => handleDeleteUser(u.id || u._id)} className="text-red-600 font-bold text-xs hover:underline">Xóa</button>
                  </td>
                </tr>
              ))}
              {filteredUsers.length === 0 && (
                <tr><td colSpan={5} className="text-center p-8 text-gray-500 font-medium">Không tìm thấy người dùng nào khớp với bộ lọc.</td></tr>
              )}
            </tbody>
          </table>
        </div>
