...
export default function AdvancedAdminDashboard() {
  ...
  const renderBookings = () => {
    ...
    const roomsWithBookings = rooms.length > 0
      ? rooms
      : [
          ...
          <div className="min-w-[1200px] h-full flex flex-col">
            ...
            {roomsWithBookings.length === 0 ? (
              ...
              roomsWithBookings.map((room) => {
                ...
                const roomBookings = MOCK_BOOKINGS.filter((b) => b.roomId === roomId);
                return (
                  <div key={roomId} className="flex border-b border-gray-100 group hover:bg-slate-50/50 transition-colors h-24">
    ...
    return (
      <div className="space-y-8 pb-10">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          >
            <div className="mb-6">
              <h3 className="text-lg font-bold text-gray-900">Tình trạng Thiết bị kho</h3>
                  ...
                  <Bar dataKey="value" name="Số lượng" fill="#3b82f6" radius={[6, 6, 0, 0]} barSize={40}>
                    {dashboardStats.equipmentStatusData.map((entry, index) => (
                      <Cell
                        key={`cell-${index}`}
                        fill={
                          index === dashboardStats.equipmentStatusData.reduce(
                            (maxIdx, current, idx, arr) => current.value > arr[maxIdx].value ? idx : maxIdx, 0
                          ) ? "#2563eb" : "#93c5fd"
                        }
                      />
                    ))}
          ...
          >
            <div className="mb-6">
              <h3 className="text-lg font-bold text-gray-900">Phân bổ người dùng</h3>
              <p className="text-sm text-gray-500">Tỷ lệ người dùng theo vai trò trong hệ thống.</p>
            </div>
            <div className="h-72 w-full flex items-center justify-center">
              <ResponsiveContainer width="100%" height="100%">
                <PieChart>
                  <Pie
                    data={dashboardStats.userRolesData.length > 0 ? dashboardStats.userRolesData : currentData.popularRooms}
                    cx="50%"
                    cy="45%"
                    innerRadius={60}
                    outerRadius={90}
                    ...
                    dataKey="value"
                  >
                    {(dashboardStats.userRolesData.length > 0 ? dashboardStats.userRolesData : currentData.popularRooms).map((entry, index) => (
